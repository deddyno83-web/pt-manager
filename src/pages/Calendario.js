...
export default function Calendario() {
  ...
  const getLessonsRemaining = (clientId) => {
    ...
    const baseCount = client.appointmentsFromPackageStart || 0;
    const used = appointments.filter(a => a.clientId === clientId).length - baseCount;
    return (client.packageLessons || 0) - used;
  };
  ...
  const handleBook = async () => {
    ...
    if (client?.type === 'individuale' && remaining !== null && remaining <= 0) {
      return showToast(`⚠️ ${client.nome} ha esaurito le lezioni del pacchetto!`, 'error');
    }
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <div>
                <h3 style={{ fontSize: 20 }}>
                  {format(selectedDate, "EEE d MMM", { locale: it }).toUpperCase()}
                </h3>
